    ...
    ui(new Ui::SettingsDialog)
{
    ...
    connect(ui->updateButton,SIGNAL(clicked()),this,SLOT(updateSlot()));
    connect(ui->serialPortInfoListBox,SIGNAL(currentIndexChanged(int)),this, SLOT(showPortInfoSlot(int)));
...
void SettingsDialog::getAvaliblePortsInfoSlot(std::vector<QStringList> *ports)
{
    ui->serialPortInfoListBox->clear();
    QStringList *portInfoList;
    for (uint i = 0; i < ports->size(); i++)
    {
      portInfoList = &(ports->at(i));

        ui->serialPortInfoListBox->addItem(portInfoList->first(), *portInfoList);
    }
}

void SettingsDialog::fillPortsSettings()
{
    // fill baud rate
...
void SettingsDialog::showPortInfoSlot(int idx)
{
    if (idx != -1) {
...
void SettingsDialog::applySlot()
{
    updateSettings();
    hide();
}

void SettingsDialog::updateSlot()
{
